...
const TravelMap = () => {
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      function generateBezierCurve(start, end, curvature = 0.2, numPoints = 80) {
      ...
      for (let i = 0; i < points.length - 1; i++) {
        curvedSegments.push(
          ...generateBezierCurve(points[i].coords, points[i + 1].coords, 0.25)
        );
      }
      ...
      points.forEach((point) => {
        ...
        `;
        tooltip.querySelector('.tooltip-photo').style.setProperty(
          '--photo-url',
